"""CrashBuyV1 A/B 过拟合检测网格（TEST，TOP10 池，独立口径 $1,000/笔）。

判据（预注册，RESEARCH 12.4）：全组合利润>0 且 **全组合 2022 ≥ 0**（复活主张须在网格上成立）
且无尖峰。用法: .venv/Scripts/python user_data/scripts/cb_batch.py [--only "q:h,..."]
"""
import json
import os
import re
...
STRAT = ROOT / "user_data" / "strategies" / "CrashBuyV1.py"
BT_DIR = ROOT / "user_data" / "backtest_results"
TMP = Path("/tmp/cb_batch")
SRC = STRAT.read_text()
...
STAKE = 1000
GRID = [(-0.12, 0.05, 0.02), (-0.12, 0.05, 0.04), (-0.12, 0.10, 0.02), (-0.12, 0.10, 0.04),
        (-0.08, 0.05, 0.02), (-0.08, 0.05, 0.04), (-0.08, 0.10, 0.02), (-0.08, 0.10, 0.04)]


def make_variant(stop, off, step):
    text = SRC
    text = re.sub(r"stoploss = -[0-9.]+", f"stoploss = {stop}", text)
    text = re.sub(r"trailing_stop_positive = 0[0-9.]+", f"trailing_stop_positive = {step}", text)
    text = re.sub(r"trailing_stop_positive_offset = 0[0-9.]+",
                  f"trailing_stop_positive_offset = {off}", text)
    name = f"CrashBuyV1_s{int(abs(stop) * 100):02d}_o{int(off * 100):02d}_t{int(step * 100):02d}"
    text = text.replace("class CrashBuyV1(IStrategy):", f"class {name}(IStrategy):")
    return name, text
...
def main():
    ...
    print(f"{'组合':<16}{'trades':>7}{'利润$':>10}{'PF':>7}  逐年（判据: 逐年全非负）")
    print("-" * 100)
    results = []
    for stop, off, step in grid:
        name, text = make_variant(stop, off, step)
        trades = run_one(name, text)
        tot, pf, ys, ytot, npair, npos = summarize(trades)
        results.append((stop, off, step, tot, pf, ys, ytot, npair, npos))
        print(f"s={stop:+.2f} off={off:.2f} step={step:.2f}{tot:>+10,.0f}{pf:>7.2f}  {ys}  品种正 {npos}/{npair}", flush=True)
    print("-" * 100)
    all_pos = all(r[2] > 0 for r in results)
    y2022 = all(r[6].get("2022", 0) >= 0 for r in results)
