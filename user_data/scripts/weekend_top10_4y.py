"""WeekendReverseV1 市值Top10 最近4年 详细简报"""
import subprocess, json, zipfile
from collections import defaultdict

PAIRS = "--pairs BTC/USDT:USDT ETH/USDT:USDT SOL/USDT:USDT XRP/USDT:USDT BNB/USDT:USDT DOGE/USDT:USDT ADA/USDT:USDT AVAX/USDT:USDT DOT/USDT:USDT LINK/USDT:USDT"
PYTHON = r"C:\Users\sieata\Documents\freqtrade\.venv\Scripts\python.exe"
CWD = r"C:\Users\sieata\Documents\freqtrade"

r = subprocess.run(
    f'{PYTHON} -m freqtrade backtesting --config user_data/config_perpetual.json --strategy WeekendReverseV1 --timerange 20220813- {PAIRS} --export trades',
...
print(f"expectancy=${s.get('expectancy', 0):,.2f}  holding_avg={s.get('holding_avg_s', 0)/3600:.1f}h")
print(f"wins={s['wins']} losses={s['losses']} draws={s['draws']}")

print("\n=== 逐品种 ===")
pairs = sorted([r2 for r2 in s['results_per_pair'] if r2['key'] != 'TOTAL'], key=lambda x: -x['profit_total_abs'])
print(f"{'pair':<10s} {'trades':>6s} {'win%':>6s} {'profit':>12s} {'dd%':>7s}")
for p in pairs:
    print(f"{p['key'].split('/')[0]:<10s} {p['trades']:>6d} {p['winrate']*100:>5.1f}% {p['profit_total_abs']:>12,.0f} {p['max_drawdown_account']*100:>6.1f}%")
...
print("\n=== 单笔分布 ===")
profs = sorted(t['profit_abs'] for t in trades)
print(f"n={len(profs)}  min=${profs[0]:,.0f}  max=${profs[-1]:,.0f}  median=${profs[len(profs)//2]:,.0f}  mean=${sum(profs)/len(profs):,.0f}")
