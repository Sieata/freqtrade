...
# 1. SHA 校验（FREEZE_H8C.md 冻结，不匹配拒绝运行；用 venv python 算，不依赖 shasum/cut）
CUR_SHA=$("$PY" -c "import hashlib,sys;print(hashlib.sha256(open(sys.argv[1],'rb').read()).hexdigest())" "$SCRIPT")
...
# 3. 跑一轮（幂等：重复运行不重复记账）
cd "$ROOT"
"$PY" -c "import os,sys;os.makedirs(sys.argv[1],exist_ok=True)" user_data/logs
TS=$("$PY" -c "import datetime;print(datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'))")
echo "[h8c_paper] $TS 运行一轮（模拟交易，FREEZE_H8C.md）"
"$PY" user_data/scripts/h8c_paper.py

# 4. cron 提示（本机不装计划任务——2026-08-29 用户指令；paper 设备用）
echo
echo "[cron 提示] 常驻监控请装在 paper 设备（24h 在线）："
echo "  17 * * * * cd <repo> && .venv/bin/python user_data/scripts/h8c_paper.py >> user_data/logs/h8c_paper.log 2>&1"
