#!/bin/bash
# FundingSqueezeV1L forward-test launcher (paper trading / dry-run) — macOS/Linux 版
# 用法（项目根目录）: ./user_data/scripts/paper_start_fs.sh
# 前提: 数据已就绪（./ensure-data.sh user_data/universe/pairs_core.txt）；与 V2 paper
#       可同机并行（独立 config/db/端口 8081），判据见 user_data/paper/FREEZE_FS.md
...
LOGFILE="$LOGDIR/paper_fs_forwardtest.log"
FROZEN_SHA="091486c816dfd392e9ecec931777d74859551c4d5ad0249aca6a4a4ec9cde978"
...
echo
echo "[OK] FS forward-test started (PID $PID)"
echo "    log:      $LOGFILE"
echo "    db:       user_data/tradesv3.dryrun.fs.sqlite"
echo "    api/ui:   http://127.0.0.1:8081  (账号见 config_paper_fs.json)"
echo "    stop:     kill $PID"
echo
