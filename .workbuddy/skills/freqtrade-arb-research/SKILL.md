...
| `arb_regime_check.py` | **年景体检**：funding 逐年/近期/逐月 + 逐合约「最优点入场」净收益（回答"某年还有没有机会"） |
...
.venv/Scripts/python.exe user_data/scripts/arb_regime_check.py                  # 年景体检
```
...
7. **上界检验（最便宜的早期一票否决）**：先抛开全部门禁规则，对**每个事件/合约强制取
   窗口内年化基差最大点**入场（= 开天眼挑时点），算净收益。若这个**上界都为负**，
   规则化回测不必再跑——结论已经定了。实测：币本位 ETHUSD 交割 2026 年 4 个合约上界
   均值 **−1.01%**（2024 年 +3.40%）→ 2026 直接判死，省掉整套 θ 扫描。
   `arb_regime_check.py` 的 ② 段就是干这个。
...
- **币本位 ETHUSD 永续×交割**：结构在（到期收敛，θ=8% 7 事件 100% 胜），但钱没有——
  ...
  **2026 上界已转负**（4 合约「最优点入场」净均值 −1.01%，其中 260327 −1.77%、260626 −2.08%）；
  按 θ=8% 门禁 2026 年正反两向**零触发**；在市 261225 正向预期净 −0.313%（折年 −1.2%）。
  → 2026 这个结构**没有用武之地**。逐年上界（单合约净均值）：2022 +1.75% / 2023 +1.07% /
  2024 +3.40% / 2025 +1.25% / **2026 −1.01%**。详见 RESEARCH 22.7。
- **资金费年景单调衰减**（正=多头付空头）：ETHUSDT 2021 +37.6% / 2022 +0.8% / 2023 +8.3% /
  2024 +13.0% / 2025 +4.9% / **2026 +1.8%**；币本位 2026 +2.8%。近 90 天回升到 +4.4%/+5.3%。
  同族里唯一还活着的只剩**纯资金费收割**（空永续+多现货，2026 年化 2~3%），
  但那是 U 本位现货×永续的活，**不需要币本位交割腿**（2026 年交割腿的基差成本 > 它能替代的现货）。
